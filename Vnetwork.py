...
class Encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        return out5, skip1, skip2, skip3, skip4
...
class VNet(nn.Module):
    ...
    def __init__(self, parameters):
        ...
        self.encoder = Encoder(self.param['query_encoder'])
        self.decoder = Decoder(self.param['decoder'])


    def forward(self, query):
        torch.autograd.set_detect_anomaly(True)
        output_encoder, skip1, skip2, skip3, skip4 = self.encoder(query)
        output = self.decoder(output_encoder, skip1, skip2, skip3, skip4)
        return output
